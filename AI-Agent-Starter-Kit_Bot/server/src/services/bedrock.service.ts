...
export class BedrockService {
  ...
  async analyzeContent(imageUrl: string) {
    const prompt = `
    ...
    Analyze this content for authenticity:
    Image: ${imageUrl}
    Verify: ownership claims, creation date, modifications`;

    const command = new InvokeModelCommand({
      modelId: process.env.AWS_MODEL_ARN,
  ...
  async detectPlagiarism(content: string) {
    ...
    Ensure the model strictly follows the JSON output format. If text includes both plagiarism and fair use elements, classify based on the dominant intent. Wait for the user to send their JSON for you to respond. DO NOT RESPOND WITH ANYTHING OTHER THAN THE SPECIFIED MODEL OUTPUT FORMAT.
    Check this content for potential plagiarism:
    ${content}
    Identify: similar works, source attribution, modification patterns`;
